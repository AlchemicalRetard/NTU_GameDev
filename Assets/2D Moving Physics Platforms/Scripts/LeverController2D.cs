...
namespace ScottEwing.MovingObjects.MovingPhysicsPlatform2D {
    public class LeverController2D : MonoBehaviour {
        ...
        [SerializeField] PhysicsPlatform2D physicsPlatform;
        ...
        void ControllBarrier() {
            if (m_Handle.transform.rotation == m_OnQuaternion) {
                if (ShouldOnlyActivateOnce && !m_IsLeverOn) {           // platform will still have start delay when lever is activated after originally 
                    physicsPlatform.doStartDelayTimerOnEnable = true;   // ... being off
                }
                else if (!ShouldOnlyActivateOnce && !HasBeenActivated && !m_IsLeverOn) {
                    physicsPlatform.doStartDelayTimerOnEnable = true;
                }
